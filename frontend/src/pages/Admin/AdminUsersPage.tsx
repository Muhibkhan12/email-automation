...
const StatCard = ({ stat }: { stat: Stat }) => {
  ...
  return (
    <div className="stat-card rounded-xl bg-[#171A21] p-4 md:p-5 border border-[#2A2E37] hover:border-[#3A3F4A]">
      <div className="flex items-start justify-between">
        <div className="flex h-8 w-8 md:h-9 md:w-9 items-center justify-center rounded-lg" style={{ background: `${stat.accent}15` }}>
          <Icon size={14} style={{ color: stat.accent }} />
        </div>
        {stat.isRatio ? (
          <span className="text-[10px] md:text-[11.5px] font-medium text-[#8B8D94]">{stat.change}</span>
        ) : (
          <span className={`flex items-center gap-0.5 text-[10px] md:text-[11.5px] font-medium ${stat.trend === "up" ? "text-[#7FD98A]" : "text-[#FF5C6C]"}`}>
            {stat.trend === "up" ? <ArrowUpRight size={12} /> : <ArrowDownRight size={12} />}
            {stat.change}
          </span>
        )}
      </div>
      <h2 className="mt-3 md:mt-4 text-xl md:text-2xl font-semibold tracking-tight text-[#E8E6E1] font-['JetBrains_Mono']">{stat.value}</h2>
...
const SenderAccountCard = ({ account }: { account: UserWithSenderAccounts["sender_accounts"][number] }) => {
  ...
  return (
    <div className="flex items-center gap-3 rounded-lg border border-[#2A2E37] bg-[#171A21] px-3.5 py-3">
      <div className="flex h-8 w-8 flex-shrink-0 items-center justify-center rounded-lg bg-[#0E1013] border border-[#2A2E37]">
        <ProviderIcon size={13} className="text-[#8B8D94]" />
      </div>

      <div className="min-w-0 flex-1">
        <div className="flex items-center gap-2">
          <p className="truncate text-[12px] md:text-[12.5px] font-medium text-[#E8E6E1]">{account.email}</p>
          <span className={`inline-flex flex-shrink-0 items-center gap-1 rounded-full px-1.5 py-0.5 text-[9px] md:text-[10px] font-medium ${style.bg} ${style.text}`}>
            <StatusIcon size={9} />
            {status}
          </span>
        </div>
        ...
        {account.warmupProgress !== undefined && (
          <p className="mt-1 text-[9px] md:text-[10px] text-[#4FA3FF]">Warming up · {account.warmupProgress}%</p>
        )}
      </div>
    </div>
  );
    ...
    return (
      <div className="flex min-h-screen bg-[#0E1013]">
        ...
        <main className="flex flex-1 items-center justify-center">
          <div className="text-sm text-[#8B8D94]">Loading users...</div>
        </main>
      </div>
    );
  }
      ...
      <div className="flex min-h-screen bg-[#0E1013]">
        ...
        <main className="flex flex-1 items-center justify-center">
          <div className="rounded-xl border border-red-500/20 bg-[#171A21] px-6 py-5">
            ...
            <p className="mt-1 text-xs text-[#8B8D94]">{error}</p>
          </div>
        </main>
      </div>
    );
      ...
      <main className="main-content flex-1 overflow-y-auto p-4 md:p-6 lg:p-8 bg-[#0E1013] h-screen">
        ...
        <div className="mb-8 flex flex-wrap items-center justify-between gap-4">
          <div>
            <div className="flex flex-wrap items-center gap-2 md:gap-2.5">
              <h1 className="text-2xl md:text-3xl font-bold tracking-tight text-[#E8E6E1] font-['Space_Grotesk']">Users</h1>
              <span className="rounded-full px-2 md:px-2.5 py-0.5 text-[10px] md:text-[11px] font-medium bg-[#FF6A39]/15 text-[#FF6A39]">Admin View</span>
            </div>
            <p className="mt-1 text-xs md:text-sm text-[#8B8D94]">Manage users and the sender accounts they send campaigns from.</p>
          </div>
        ...
        <div className="mb-6 flex flex-wrap items-center justify-between gap-4">
          <div className="flex flex-wrap items-center gap-3">
            ...
            <select value={statusFilter} onChange={(e) => setStatusFilter(e.target.value)} className="rounded-lg border border-[#2A2E37] bg-[#171A21] px-3 py-2 text-xs md:text-sm text-[#C7C9CE] outline-none focus:border-[#FF6A39]">
              {FILTERS.map((f) => <option key={f}>{f}</option>)}
            </select>

            <select value={roleFilter} onChange={(e) => setRoleFilter(e.target.value)} className="rounded-lg border border-[#2A2E37] bg-[#171A21] px-3 py-2 text-xs md:text-sm text-[#C7C9CE] outline-none focus:border-[#FF6A39]">
        ...
        <div className="rounded-xl bg-[#171A21] border border-[#2A2E37] overflow-hidden">
                      ...
                      <tr className={`group mf-row transition border-t border-[#2A2E37] ${selectedUsers.has(user.id) ? "is-selected" : ""}`}>
                        ...
                        <td className="px-3 py-3.5">
                          <div className="flex items-center gap-3">
                            ...
                            <div>
                              <p className="text-[12px] md:text-[13.5px] font-medium text-[#E8E6E1]">{user.username}</p>
                        ...
                        <td className="px-3 py-3.5">
                          <span className={`inline-flex items-center gap-1.5 rounded-full px-2 py-0.5 text-[10px] md:text-[11px] font-medium ${statusStyle.bg} ${statusStyle.text}`}>
                            <span className={`h-1.5 w-1.5 rounded-full ${statusStyle.dot}`} /> {status}
                          </span>
                        </td>
                        ...
                        <td className="px-3 py-3.5 text-[11px] md:text-[13px] text-[#C7C9CE] font-['JetBrains_Mono']">—</td>

                        <td className="px-3 py-3.5">
                          <span className="text-[10px] md:text-[12px] text-[#8B8D94]">{new Date(user.created_at).toLocaleDateString()}</span>
                        </td>

                        <td className="px-4 md:px-5 py-3.5 text-right">
                          <div className="flex items-center justify-end gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
                            <button className="p-1 rounded hover:bg-[#0E1013] text-[#8B8D94] hover:text-[#E8E6E1] transition"><Edit size={14} /></button>
                            <button className="p-1 rounded hover:bg-[#0E1013] text-[#8B8D94] hover:text-[#E8E6E1] transition"><Key size={14} /></button>
                            <button className="p-1 rounded hover:bg-[#0E1013] text-[#8B8D94] hover:text-[#E8E6E1] transition"><MoreHorizontal size={14} /></button>
                          </div>
                        </td>
                      </tr>
          ...
          <div className="flex flex-wrap items-center justify-between gap-3 p-4 md:p-5 border-t border-[#2A2E37]">
            ...
            <div className="flex items-center gap-1.5">
              <button className="px-3 py-1.5 rounded-lg border border-[#2A2E37] text-[#C7C9CE] text-xs hover:bg-[#1B1E24] transition"><ChevronLeft size={14} /></button>
              ...
              <button className="px-3 py-1.5 rounded-lg border border-[#2A2E37] text-[#C7C9CE] text-xs hover:bg-[#1B1E24] transition"><ChevronRight size={14} /></button>
            </div>
          </div>
        </div>
      </main>
