...
const C = {
  bg: '#F4F5F8', surface: '#FFFFFF', sidebar: '#11141B', sidebarHover: '#1B2030',
  sidebarActive: '#20263A', primary: '#2F6FED', primarySoft: '#EAF0FE',
...
interface NavChild { id: string; label: string; icon: LucideIcon; }
interface NavItem { id: string; label: string; icon: LucideIcon; children?: NavChild[]; }
type CampaignStatus = 'Delivered' | 'Sending' | 'Failed' | 'Scheduled';
type PulseStatus = 'success' | 'pending' | 'failed';
interface Campaign { name: string; status: CampaignStatus; recipients: number; open: string; date: string; }
interface PulseEvent { status: PulseStatus; text: string; }
interface DeliveryPoint { day: string; sent: number; }
interface StatCardProps { icon: LucideIcon; label: string; value: string; delta: string; positive: boolean; accent: string; }
interface NavRowProps { item: NavItem | NavChild; depth: 0 | 1; active: string; expanded: boolean; onToggle: (id: string) => void; onSelect: (id: string) => void; }
...
function findLabel(id: string): { label: string; parent: string | null } {
...
const deliveryData: DeliveryPoint[] = [
  { day: 'Jul 24', sent: 3200 }, { day: 'Jul 25', sent: 4100 }, { day: 'Jul 26', sent: 3800 },
  { day: 'Jul 27', sent: 5200 }, { day: 'Jul 28', sent: 4700 }, { day: 'Jul 29', sent: 6100 },
  { day: 'Jul 30', sent: 5800 }, { day: 'Jul 31', sent: 7200 }, { day: 'Aug 1', sent: 6900 },
  { day: 'Aug 2', sent: 8100 }, { day: 'Aug 3', sent: 7600 }, { day: 'Aug 4', sent: 8800 },
...
const statusStyle = (status: CampaignStatus): { bg: string; fg: string } => {
  switch (status) {
...
const dotColor = (status: PulseStatus): string =>
  status === 'success' ? C.success : status === 'failed' ? C.danger : C.warning;

function StatCard({ icon: Icon, label, value, delta, positive, accent }: StatCardProps) {
  return (
    <div className="flex-1 min-w-[190px] rounded-xl p-5" style={{ background: C.surface, border: `1px solid ${C.border}` }}>
      <div className="flex items-center justify-between mb-4">
        ...
        <span
          ...
          style={{ color: positive ? C.success : C.danger, background: positive ? C.successSoft : C.dangerSoft }}
        >
          {positive ? <ArrowUpRight size={12} /> : <ArrowDownRight size={12} />}
          {delta}
        </span>
      </div>
      <p style={{ fontFamily: C.mono, color: C.text }} className="text-2xl font-semibold tracking-tight">{value}</p>
      <p style={{ color: C.textSoft }} className="text-sm mt-1">{label}</p>
    </div>
  );
}

function NavRow({ item, depth, active, expanded, onToggle, onSelect }: NavRowProps) {
  ...
  return (
    <button
      ...
      style={{
        ...
        fontSize: depth === 0 ? 13.5 : 13, fontWeight: depth === 0 ? 500 : 400, fontFamily: C.body,
      }}
...
export default function App() {
  ...
  const toggleSection = (id: string) => {
    ...
    if (item && !item.children) setActive(id);
  };

  const { label: pageLabel, parent: pageParent } = findLabel(active);

  return (
    <div className="h-screen w-full flex overflow-hidden" style={{ background: C.bg, fontFamily: C.body }}>
      <style>{`
      ...
      <aside className="h-full flex flex-col shrink-0" style={{ width: 258, background: C.sidebar, borderRight: `1px solid #1E2330` }}>
        ...
        <nav className="flex-1 overflow-y-auto px-3 py-4 flex flex-col gap-0.5">
          {NAV.map((item) => (
            <div key={item.id}>
              <NavRow item={item} depth={0} active={active} expanded={!!expanded[item.id]} onToggle={toggleSection} onSelect={setActive} />
              {item.children && expanded[item.id] && (
                <div className="flex flex-col gap-0.5 mt-0.5 mb-1">
                  {item.children.map((child) => (
                    <NavRow key={child.id} item={child} depth={1} active={active} expanded={false} onToggle={() => {}} onSelect={setActive} />
                  ))}
                </div>
              )}
            </div>
      ...
      <main className="flex-1 h-full overflow-y-auto">
        ...
        >
          <div className="leading-tight">
            <p style={{ fontFamily: C.mono, color: C.textFaint, fontSize: 11 }}>{pageParent ? `${pageParent} /` : 'Menu /'}</p>
            <p style={{ fontFamily: C.display, color: C.text, fontSize: 15.5, fontWeight: 600 }}>{pageLabel}</p>
          </div>
          <div className="flex items-center gap-3">
            <div className="hidden md:flex items-center gap-2 px-3 rounded-lg" style={{ background: C.surface, border: `1px solid ${C.border}`, height: 36, width: 240 }}>
              <Search size={14} color={C.textFaint} />
              <input placeholder="Search campaigns, contacts…" className="bg-transparent outline-none text-sm w-full" style={{ color: C.text }} />
            </div>
            <button className="relative w-9 h-9 rounded-lg flex items-center justify-center" style={{ background: C.surface, border: `1px solid ${C.border}` }}>
              ...
              <span className="absolute top-1.5 right-1.5 w-2 h-2 rounded-full" style={{ background: C.danger, border: `1.5px solid ${C.surface}` }} />
            </button>
          </div>
        </div>

        {active === 'dashboard' ? (
          <div className="px-8 py-7 flex flex-col gap-6 max-w-[1180px]">
            <div>
              <h1 style={{ fontFamily: C.display, color: C.text, fontSize: 24, fontWeight: 700, letterSpacing: '-0.01em' }}>Good afternoon, Ayesha</h1>
              <p style={{ color: C.textSoft, fontSize: 13.5 }} className="mt-1">Here's what's moving across your sends today.</p>
            </div>
              ...
              <div className="flex flex-col gap-2.5">
                {pulseEvents.map((e, i) => (
                  <div key={i} className="flex items-center gap-3">
                    ...
                    <span style={{ fontFamily: C.mono, fontSize: 12, color: C.textSoft }}>{e.text}</span>
                  </div>
                ))}
              </div>
            </div>
            ...
            <div className="rounded-xl p-5" style={{ background: C.surface, border: `1px solid ${C.border}` }}>
              <div className="flex items-center justify-between mb-4">
                <p style={{ fontFamily: C.display, color: C.text, fontSize: 13.5, fontWeight: 600 }}>Emails delivered — last 14 days</p>
                <span style={{ fontFamily: C.mono, color: C.textFaint, fontSize: 11 }}>daily volume</span>
              </div>
              <div style={{ height: 220 }}>
                <ResponsiveContainer width="100%" height="100%">
                  <AreaChart data={deliveryData} margin={{ top: 4, right: 4, left: -20, bottom: 0 }}>
                    ...
                    <XAxis dataKey="day" tick={{ fontSize: 10.5, fill: C.textFaint, fontFamily: C.mono }} axisLine={false} tickLine={false} interval={1} />
                    <YAxis tick={{ fontSize: 10.5, fill: C.textFaint, fontFamily: C.mono }} axisLine={false} tickLine={false} />
                    <Tooltip contentStyle={{ borderRadius: 8, border: `1px solid ${C.border}`, fontFamily: C.mono, fontSize: 12 }} />
              ...
              <table className="w-full text-left" style={{ fontSize: 13 }}>
                <thead>
                  <tr style={{ color: C.textFaint, fontSize: 11 }}>
                    ...
                    <th className="font-medium px-5 py-2.5">Recipients</th>
                    <th className="font-medium px-5 py-2.5">Open rate</th>
                  ...
                  {campaigns.map((c, i) => {
                    ...
                    return (
                      <tr key={i} style={{ borderTop: `1px solid ${C.border}` }}>
                        <td className="px-5 py-3" style={{ color: C.text, fontWeight: 500 }}>{c.name}</td>
                        <td className="px-5 py-3">
                          <span className="px-2 py-0.5 rounded-full text-xs font-medium" style={{ background: s.bg, color: s.fg }}>{c.status}</span>
                        </td>
                        <td className="px-5 py-3" style={{ fontFamily: C.mono, color: C.textSoft }}>{c.recipients.toLocaleString()}</td>
                        <td className="px-5 py-3" style={{ fontFamily: C.mono, color: C.textSoft }}>{c.open}</td>
                        <td className="px-5 py-3" style={{ color: C.textFaint }}>{c.date}</td>
                      </tr>
                    );
                  })}
                </tbody>
          ...
          <div className="px-8 py-16 flex flex-col items-center justify-center text-center" style={{ minHeight: '60vh' }}>
            <div className="w-14 h-14 rounded-2xl flex items-center justify-center mb-4" style={{ background: C.primarySoft }}>
              <LayoutDashboard size={22} color={C.primary} />
            </div>
            <p style={{ fontFamily: C.display, color: C.text, fontSize: 18, fontWeight: 600 }}>{pageLabel}</p>
