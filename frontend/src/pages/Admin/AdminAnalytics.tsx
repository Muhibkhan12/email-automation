      ...
      <main className="main-content flex-1 overflow-y-auto p-4 md:p-6 lg:p-8 bg-[#0E1013] h-screen">
        ...
        <div className="mb-8 flex flex-wrap items-center justify-between gap-4">
          ...
          <div className="flex flex-wrap items-center gap-3">
            <button className="flex items-center gap-2 rounded-lg border border-[#2A2E37] bg-[#171A21] px-3 md:px-4 py-2 md:py-2.5 text-xs md:text-sm text-[#C7C9CE] hover:border-[#3A3F4A] transition">
              <Download size={14} />
              Export
            </button>
            <select
        ...
        {/* Metrics Grid */}
        <div className="mb-6 grid grid-cols-1 gap-4 md:gap-5 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-6">
          {metrics.map((metric) => {
            const Icon = metric.icon;
            return (
              <div
                key={metric.title}
                className="stat-card rounded-xl bg-[#171A21] p-4 border border-[#2A2E37] hover:border-[#3A3F4A] transition-all"
              >
                ...
                <p className="mt-2 text-xl font-semibold tracking-tight text-[#E8E6E1] font-['JetBrains_Mono']">
                  {metric.value}
                </p>
                <p className="mt-0.5 text-[10px] text-[#8B8D94]">{metric.title}</p>
              </div>
            );
          })}
        </div>
        ...
        <div className="mb-6 grid grid-cols-1 gap-4 md:gap-5 lg:grid-cols-3">
            ...
            <div className="flex flex-wrap items-center justify-between gap-2 mb-4">
              ...
              <div className="flex items-center gap-3">
                <div className="flex items-center gap-1.5">
                  <span className="h-2 w-2 rounded-full bg-[#FF6A39]" />
                  <span className="text-[10px] text-[#8B8D94]">Sent</span>
                </div>
                <div className="flex items-center gap-1.5">
                  <span className="h-2 w-2 rounded-full bg-[#7FD98A]" />
                  <span className="text-[10px] text-[#8B8D94]">Opened</span>
                </div>
                <div className="flex items-center gap-1.5">
                  <span className="h-2 w-2 rounded-full bg-[#FFC24B]" />
                  <span className="text-[10px] text-[#8B8D94]">Clicked</span>
                </div>
              </div>
            </div>
          ...
          <div className="rounded-xl bg-[#171A21] p-4 md:p-6 border border-[#2A2E37]">
            <h2 className="text-sm font-semibold text-[#E8E6E1] font-['Space_Grotesk'] mb-4">
              Engagement Breakdown
            </h2>
              ...
              <div className="flex-1 w-full space-y-2">
                {engagementData.map((item) => (
                  <div key={item.name} className="flex items-center justify-between">
                    <span className="flex items-center gap-2 text-xs text-[#C7C9CE]">
                      <span className="h-2 w-2 rounded-full" style={{ background: item.color }} />
                      {item.name}
                    </span>
                    <span className="text-xs font-medium text-[#E8E6E1] font-['JetBrains_Mono']">
                      {item.value}%
                    </span>
                  </div>
                ))}
        ...
        <div className="mb-6 grid grid-cols-1 gap-4 md:gap-5 lg:grid-cols-3">
          ...
          <div className="rounded-xl bg-[#171A21] p-4 md:p-6 border border-[#2A2E37]">
            ...
            {/* Best Time */}
            <div className="mt-4 pt-4 border-t border-[#2A2E37]">
              <div className="flex items-center gap-2">
                <Zap size={14} className="text-[#FFC24B]" />
                <span className="text-xs text-[#8B8D94]">Best Time to Send</span>
              </div>
              <p className="mt-1 text-sm font-medium text-[#E8E6E1]">2:00 PM - 4:00 PM</p>
              <p className="text-[10px] text-[#8B8D94]">Peak engagement window</p>
            </div>
          </div>
        </div>
          ...
          <div className="overflow-x-auto">
            <table className="w-full text-left min-w-[500px]">
              <thead className="text-[10px] md:text-[11px] uppercase tracking-wide text-[#8B8D94]">
                <tr>
                  <th className="px-4 md:px-5 py-3 font-medium">Workspace</th>
                  <th className="px-3 py-3 font-medium">Emails Sent</th>
                ...
                {topWorkspaces.map((workspace, i) => (
                  ...
                  >
                    ...
                    <td className="px-3 py-3.5">
                      <div className="flex items-center gap-2">
                        <span className="text-[11px] md:text-[13px] text-[#C7C9CE] font-['JetBrains_Mono']">
                          {workspace.openRate}
                        </span>
