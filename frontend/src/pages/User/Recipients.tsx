...
const Recipients = () => {
  ...
  // Map recipient_id -> send log status/sent_at, straight from backend data
  ...
  const summary = useMemo(() => {
    ...
    const pending = recipients.filter((r) => r.status === "Pending" || r.status === "Queued" || r.status === "Sending").length;
    return { total: recipients.length, sent, failed, pending };
  }, [recipients]);

  return (
    <div className="flex min-h-screen overflow-hidden bg-[#0B0E12]" style={{ fontFamily: FONT.body }}>
      <style>{`
      ...
      <div className={`fixed lg:sticky top-0 z-50 h-screen flex-shrink-0 transition-transform duration-250 ease-out ${sidebarOpen ? 'translate-x-0' : '-translate-x-full lg:translate-x-0'} sidebar-slide`}>
      ...
      <main className="main-content flex-1 overflow-y-auto p-3 md:p-4 lg:p-6 xl:p-8 bg-[#12151B] h-screen w-full">
          ...
          <div className="flex items-center gap-3 md:gap-4">
            ...
            <div>
              ...
              <p className="mt-0.5 md:mt-1 text-[10px] md:text-xs lg:text-sm text-[#9BA0A8]">
                {activeCampaign ? `Recipients for "${activeCampaign.campaign_name}"` : "Select a campaign to view its recipients."}
              </p>
            </div>
          </div>
        </div>

        {/* Loading / error from campaign fetch */}
        {loading && (
        ...
        {/* No campaign selected — honest empty state, no fabricated global list */}
        {!loading && !error && !campaignId && (
          <div className="text-center py-16 rounded-xl border border-[#2A2E37] bg-[#1B1E24]">
            ...
            <p className="text-sm text-[#9BA0A8] mb-3">No campaign selected.</p>
            <button
        ...
        {/* Campaign selected but not found (e.g. wrong id, or not this user's campaign) */}
        {!loading && !error && campaignId && !activeCampaign && (
          <div className="text-center py-16 rounded-xl border border-[#2A2E37] bg-[#1B1E24]">
            <p className="text-sm text-[#9BA0A8]">Campaign not found.</p>
          </div>
        )}
          ...
          <>
            ...
            <div className="flex items-center justify-between mb-3 md:mb-4 px-3 py-2 rounded-lg border border-[#FF6A39]/30 bg-[#FF6A39]/10">
              <p className="text-xs text-[#E8E6E1] truncate">
                Showing recipients for <span className="font-semibold">{activeCampaign.campaign_name}</span>
              </p>
              <button
            ...
            {/* Summary cards — real counts derived from this campaign's recipients */}
                ...
                <table className="w-full text-left min-w-[500px] md:min-w-[600px]">
                  <thead>
                    <tr className="text-[8px] md:text-[9px] lg:text-[11px] uppercase tracking-wider text-[#6B727C]">
                      ...
                      <th className="px-1.5 md:px-2 lg:px-3 py-1.5 md:py-2 lg:py-2.5 font-medium">Sent at</th>
                    </tr>
                  </thead>
                  <tbody>
                    {filtered.length === 0 ? (
                      <tr>
                        <td colSpan={3} className="px-2 md:px-3 lg:px-5 py-6 md:py-8 lg:py-12 text-center text-[9px] md:text-xs lg:text-sm text-[#6B727C]">
                          No recipients match.
                        </td>
                      </tr>
                    ) : (
                      filtered.map((r) => {
                        ...
                        return (
                          <tr key={r.id} className="border-t border-[#2A2E37] hover:bg-[#1B1E24] transition-colors">
                            <td className="px-2 md:px-3 lg:px-5 py-1.5 md:py-2 lg:py-3">
                              ...
                              <p className="text-[7px] md:text-[8px] lg:text-[12px] text-[#6B727C] truncate">{r.email}</p>
                            </td>
